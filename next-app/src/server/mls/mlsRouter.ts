...
export const mlsRouter = router({
  ...
  /**
   * getSyncStatus — MLS sync overview (total properties, last sync, etc.).
   * Called by: Home dashboard
   */
  getSyncStatus: protectedProcedure.query(async () => {
    const data = await ldsGet<{
  ...
  /**
   * triggerSync — ask listing-data-service to start an MLS sync.
   * Called by: Home dashboard
   */
  triggerSync: protectedProcedure.mutation(async () => {
    const data = await ldsPost<{ success: boolean; started: boolean }>(
      "/api/v1/sync/trigger"
    );
    return data ?? { success: false, started: false };
  }),

  /**
   * embeddingStatus — check vector embedding generation progress.
   * Called by: CMAStudio
   */
  embeddingStatus: protectedProcedure.query(async () => {
    const data = await ldsGet<{
      totalProperties?: number;
  ...
  /**
   * triggerEmbedding — ask listing-data-service to generate embeddings.
   * Called by: CMAStudio
   */
  triggerEmbedding: protectedProcedure.mutation(async () => {
    const data = await ldsPost<{ success: boolean; started: boolean; message?: string }>(
      "/api/v1/embeddings/trigger"
    );
    return data ?? { success: false, started: false, message: "" };
  }),
});

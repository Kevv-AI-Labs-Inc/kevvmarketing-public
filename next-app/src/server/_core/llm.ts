/**
 * LLM Service — Dual Provider (Gemini / OpenAI)
 *
 * Provides LLM text generation with auto provider selection
 * via AI_PROVIDER env var. Models configurable via GEMINI_MODEL / OPENAI_MODEL.
 */
...
async function invokeGemini(params: InvokeParams): Promise<InvokeResult> {
  ...
  const geminiHistory = otherMessages.slice(0, -1).map((m) => ({
...
const normalizeResponseFormat = ({
  ...
  if (explicitFormat) {
    if (explicitFormat.type === "json_schema" && !explicitFormat.json_schema?.schema) {
      throw new Error("json_schema requires a defined schema object");
    }
...
async function invokeOpenAI(params: InvokeParams): Promise<InvokeResult> {
  if (!resolveOpenAiApiKey({ scope: "chat" })) {
    throw new Error(
      "OpenAI chat key is not configured. Set OPENAI_API_KEY or AZURE_OPENAI_CHAT_API_KEY."
    );
  }
  ...
  const response = await fetch(apiUrl, {
    method: "POST",
    headers: buildOpenAiAuthHeaders({ scope: "chat" }),
